...
export async function initHero(canvas, motion) {
  ...
  const front = new THREE.MeshPhysicalMaterial({ color: 0xa7b6ef, metalness: .87, roughness: .2, clearcoat: 1, clearcoatRoughness: .16, envMapIntensity: 1.2 });
  const edge = new THREE.MeshStandardMaterial({ color: 0x647dbd, metalness: .92, roughness: .22, envMapIntensity: 1.4 });
  ...
  const key = new THREE.DirectionalLight(0xd9e4ff, 4); key.position.set(-3, 5, 5); scene.add(key);
  const rim = new THREE.DirectionalLight(0x506fff, 5); rim.position.set(5, 1, -2); scene.add(rim);
  ...
  for (let i = 0; i < 90; i++) { positions[i * 3] = Math.sin(i * 12.9898) * 5; positions[i * 3 + 1] = Math.cos(i * 4.1414) * 4.5; positions[i * 3 + 2] = -2 - (i % 7) / 2; }
  const pointsGeometry = new THREE.BufferGeometry(); pointsGeometry.setAttribute('position', new THREE.BufferAttribute(positions, 3));
  const points = new THREE.Points(pointsGeometry, new THREE.PointsMaterial({ color: 0x9fb8ff, size: .017, transparent: true, opacity: .45, depthWrite: false })); scene.add(points);
  ...
  const render = time => {
    ...
    knight.rotation.y = -.42 + Math.sin(t) * .13 + pointerX * .15;
    knight.rotation.x = -.08 + pointerY * .08;
    knight.rotation.z = -.11 + Math.sin(t * .7) * .025;
...
export function initMethod(canvas, motion) {
  ...
  const total = 360, positions = Array.from({ length: total }, () => ({ x: 0, y: 0 }));
  ...
  function target(index) {
    ...
    if (step === 0) { const radius = .11 + (index % 4) * .065; return [Math.cos(angle) * radius, Math.sin(angle) * radius]; }
    if (step === 1) { const line = index % 3; return [ratio * .68 - .34, (line - 1) * .13 + Math.sin(ratio * Math.PI) * (line - 1) * .08]; }
  ...
  function draw() {
    ...
    positions.forEach((position, index) => {
      ...
      ctx.fillStyle = index % 5 === 0 ? '#e3e9ff' : '#9faeff'; ctx.globalAlpha = .5 + (index % 4) * .16;
      ctx.beginPath(); ctx.arc(centerX + position.x * scale, centerY + position.y * scale, index % 5 === 0 ? 1.7 : 1.05, 0, Math.PI * 2); ctx.fill();
    }); ctx.globalAlpha = 1;
